...
function Dashboard() {
  ...
  // Extract unique categories for filter tabs, merging typos dynamically
  const categories = ['All', ...new Set(allVehicles.map((v) => {
    const cat = v.category.trim();
    if (cat.toLowerCase() === 'elecric') return 'Electric';
  ...
  const displayedVehicles = activeCategory === 'All'
    ...
    : vehicles.filter((v) => {
        ...
        if (target === 'electric') {
          return cat === 'electric' || cat === 'elecric';
        }
        return cat === target;
      });
  ...
  const getCarIllustration = (make, category) => {
    const isElectric = category.toLowerCase().includes('electric') || category.toLowerCase().includes('ev') || category.toLowerCase().includes('elecric');
      ...
      <header className="sticky top-0 z-40 w-full glass-panel border-b border-white/5 px-6 py-4 flex flex-col sm:flex-row justify-between items-center gap-4">
        ...
        <div className="flex items-center gap-4">
          <span className="text-xs font-bold uppercase tracking-widest text-[#dfa959] bg-[#dfa959]/10 border border-[#dfa959]/20 px-3 py-1.5 rounded-lg flex items-center gap-1.5">
            ...
            {userRole === 'ADMIN' ? 'System Administrator' : 'Showroom Access'}
          </span>
          {userRole === 'ADMIN' && (
            <a
              href="/admin"
              className="text-xs font-extrabold uppercase tracking-wider text-slate-300 hover:text-white transition-colors border border-white/10 px-4 py-2 rounded-xl bg-white/5"
            >
              Go to Admin
            </a>
          )}
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-rose-600 hover:bg-rose-500 active:bg-rose-700 rounded-xl text-white font-extrabold text-xs uppercase tracking-wider transition-all cursor-pointer shadow-lg shadow-rose-950/20"
          >
      ...
      <div className="max-w-7xl mx-auto px-6 mt-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          ...
          <div className="bg-[#121216]/65 border border-white/5 p-6 rounded-2xl flex items-center justify-between shadow-xl relative overflow-hidden group hover:border-[#dfa959]/20 transition-all duration-300">
            ...
            <div className="w-12 h-12 bg-emerald-500/10 rounded-xl border border-emerald-500/20 flex items-center justify-center text-emerald-400 text-xl group-hover:scale-110 transition-transform">
          ...
          <form onSubmit={handleManualSearch} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2.5 bg-[#0a0a0d] rounded-xl text-white text-sm border border-white/5 focus:border-[#dfa959] focus:outline-none transition-all placeholder:text-slate-500"
              ...
              <div>
                ...
                <div className="relative">
                  <span className="absolute inset-y-0 left-0 pl-3.5 flex items-center text-slate-500"><i className="fa-solid fa-signature text-xs"></i></span>
                  <input
                    type="text"
                    id="model"
                    value={modelQuery}
              ...
              <div className="flex gap-4 items-end justify-end pt-5">
                {hasActiveFilters && (
                  <button
                    type="button"
                    onClick={handleClearSearch}
                    className="px-5 py-3 border border-rose-500/20 hover:border-rose-500/40 hover:bg-rose-500/10 text-rose-400 active:scale-[0.98] rounded-xl text-xxs font-black uppercase tracking-widest transition-all cursor-pointer"
                  >
                    Reset
                  </button>
                )}
                <button
                  type="submit"
                  className="flex-grow px-6 py-3.5 bg-gradient-to-r from-[#fcd34d] via-[#dfa959] to-[#b5893d] hover:brightness-110 active:scale-[0.98] rounded-xl text-[#070709] font-black text-xs uppercase tracking-widest transition-all cursor-pointer shadow-lg shadow-amber-500/5"
                >
                  Search Showroom
                </button>
              </div>
        ...
        <div className="mb-8">
          <p className="text-xxs font-bold mb-3.5 text-slate-500 uppercase tracking-widest">Quick Showroom Filter</p>
              ...
              >
                <div>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="px-3 py-1 bg-white/5 text-slate-300 border border-white/5 rounded-lg text-xxs font-black uppercase tracking-widest">
                      {vehicle.category}
                    </span>
                  </div>
                ...
                <button
                  ...
                  className="w-full mt-6 py-3.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:brightness-110 disabled:from-slate-800/80 disabled:to-slate-800/80 disabled:text-slate-500 disabled:cursor-not-allowed rounded-xl text-white font-black text-xs uppercase tracking-widest transition-all shadow-md active:scale-[0.98] cursor-pointer"
                >
        ...
        <div className="mt-20 pt-10 border-t border-white/5">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 text-center sm:text-left">
            ...
            <div className="bg-[#121216] border border-white/5 px-4 py-2.5 rounded-xl self-center">
          ...
          {purchasedItems.length === 0 ? (
            <div className="text-center text-slate-500 py-16 bg-[#121216]/10 border border-white/5 border-dashed rounded-3xl">
              <i className="fa-solid fa-receipt text-3xl text-slate-700 mb-4"></i>
              <p className="font-extrabold uppercase tracking-wider text-xs text-slate-400">No assets acquired yet</p>
              <p className="text-xxs text-slate-500 mt-1">Make your first vehicle purchase above to log records into your secure garage.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {purchasedItems.map((item) => (
                <div key={item.id} className="bg-[#121216]/65 border border-white/5 p-6 rounded-2xl flex items-center justify-between shadow-lg hover:border-[#dfa959]/20 transition-colors">
                  <div>
                    ...
                    <p className="text-xxs text-slate-500 mt-2 font-bold uppercase tracking-wider">
                      <i className="fa-solid fa-calendar-check mr-1.5 text-slate-500"></i>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xxs text-slate-400 font-bold uppercase tracking-wider mt-1">{item.quantity} {item.quantity === 1 ? 'unit' : 'units'}</p>
                  </div>
                </div>
              ))}
            </div>
